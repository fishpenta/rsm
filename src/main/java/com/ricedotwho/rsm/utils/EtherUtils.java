...
public class EtherUtils implements Accessor {
    ...
    private boolean isAir(BlockPos pos) {
        if (Minecraft.getInstance().level == null
                || !Minecraft.getInstance().level.hasChunk(pos.getX() >> 4, pos.getZ() >> 4))
            return true;
    ...
    public Pos predictTeleport(int distance, Pos start, float yaw, float pitch) {
        ...
        while (true) {
            if ((double) i < (double) distance * STEPS) {
                ...
                cur.selfAdd(forward.multiply(-STEPS));
                if (i == 0 || !isIgnored(cur) && inBB(cur)
                        || !isIgnored(cur.add(0.0, 1.0, 0.0)) && inBB(cur.add(0.0, 1.0, 0.0))) {
                    return null;
                }
            }
    ...
    //
    //
    // public Pos predictTeleport(Vec3 start, float yaw, float pitch, float
    // distance) {
    ...
    // return null;
    // }
    // return new Pos(Mth.floor(cur.x) + 0.5, Mth.floor(cur.y), Mth.floor(cur.z) +
    // 0.5);
    // }
    // }
    // if ((!isIgnored2(cur) && inBB(cur)) || (!isIgnored2(cur.add(0, 1, 0)) &&
    // inBB(cur.add(0, 1, 0)))) {
    // cur = cur.add(forward.multiply(-STEPS));
    // if (i == 0 || (!isIgnored(cur) && inBB(cur)) || (!isIgnored(cur.add(0, 1, 0))
    // && inBB(cur.add(0, 1, 0)))) {
    // return null;
    // }
    // stepsTaken = i;
    ...
    // Vec3 pos = start.add(Pos.fromRotation(pitch,
    // yaw).multiply(multiplicationFactor).asVec3());
    // if ((!isIgnored(cur) && inBB(cur)) || (!isIgnored(cur.add(0, 1, 0)) &&
    // inBB(cur.add(0, 1, 0)))) return null;
    // return new Pos(Mth.floor(pos.x) + 0.5, Mth.floor(pos.y), Mth.floor(pos.z) +
    // 0.5);
    // }
    //
    // @Nullable
    // private Pos predictTeleport2(Vec3 start, float yaw, float pitch, float
    // distance) {
    // Pos cur = new Pos(start);
    //
    // Pos forward = Pos.fromRotation(pitch, yaw)
    ...
    //
    // Pos pos = new Pos(start).add(Pos.fromRotation(pitch,
    // yaw).multiply(multiplicationFactor)
    // );
    //
